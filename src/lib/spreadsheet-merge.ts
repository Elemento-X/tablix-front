...
import { createWorkbookFromJson, addSheetFromJson, type RowData } from '@/lib/excel-utils'
 ...
 * XLS/XLSX/XLSM: SheetJS in an isolated Web Worker (CVE-2023-30533 isolated).
 *
 * Reading goes through SheetJS — not ExcelJS — because ExcelJS rejects valid
 * files whose internal [Content_Types].xml differs from Excel's own output
 * (e.g. generated by pandas/openpyxl/LibreOffice). ExcelJS stays in writing only.
 */
async function parseFileData(file: File): Promise<RowData[]> {
  ...
  // Excel (.xls, .xlsx, .xlsm) — SheetJS in Web Worker
...
export async function mergeSpreadsheets(options: MergeOptions): Promise<MergeResult> {
  ...
  // Parse every file first so we can enforce the row limit against the real
  // total and report it precisely (descriptive error, never generic).
  const parsedFiles: RowData[][] = []
  let totalRows = 0
  ...
  // Filter to only selected columns, sanitize values, and concatenate.
